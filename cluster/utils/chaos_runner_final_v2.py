...
MODE_PRESETS = {
    "smoke": {"events": 100, "kill_prob": 0.0, "delay_min": 0.0, "delay_max": 0.02, "retries": 3, "send_workers": 2, "drain_timeout": 15.0},
    "benchmark": {"events": 2000, "kill_prob": 0.05, "delay_min": 0.0, "delay_max": 0.01, "retries": 10, "send_workers": 4, "drain_timeout": 30.0},
    "torture": {"events": 10000, "kill_prob": 0.05, "delay_min": 0.1, "delay_max": 0.2, "retries": 25, "send_workers": 4, "drain_timeout": 60.0},
}
...
def main():
    print(f"[MAIN] starting UNIVERSAL chaos torture final v2 ({ARGS.mode})")
    print_test_config()
    workers = []
