...
def parse_args():
    parser = argparse.ArgumentParser(description="Universal chaos torture runner v3")
...
def process_pending_event(pending: PendingEvent):
        ...
        if result == "retryable_error" or r.status_code >= 500:
            ...
            if pending.retries_done + 1 >= MAX_RETRIES:
                ...
                stat_unexpected_failure(
                    ...
                    payload=payload,
                    reason="dropped_after_retries_retryable_response",
                )
    ...
    if pending.retries_done + 1 >= MAX_RETRIES:
        ...
        stat_unexpected_failure(
            ...
            attempt=attempt_number,
            node=node,
            reason="dropped_after_retries_exception",
            payload={"tried_nodes": list(pending.tried_nodes)},
        )
        print(f"[DROP] {i:03} event dropped after retries")
        return False
...
def main():
    print("[MAIN] starting UNIVERSAL chaos torture v3")
    print_test_config()
